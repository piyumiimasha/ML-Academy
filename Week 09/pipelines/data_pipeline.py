...
def save_processed_data(
    ...
    if output_format in ["csv", "both"]:
        ...
        # Enforce exact column order as specified in guidelines
        ...
        available_columns = [col for col in expected_column_order if col in X_train_pd.columns]
        X_train_pd = X_train_pd[available_columns]
        X_test_pd = X_test_pd[available_columns]
        
        logger.info(f"✓ Column order enforced: {list(X_train_pd.columns)}")
...
def data_pipeline(
    ...
    try:
        ...
        # Drop rows with missing Gender values (no Gender_Unknown column)
        initial_gender_count = df.count()
        df = df.dropna(subset=['Gender'])
        gender_dropped = initial_gender_count - df.count()
        logger.info(f"✓ Dropped {gender_dropped} rows with missing Gender values")

    ...
    def preprocess_input(self, data: Dict[str, Any]) -> pd.DataFrame:
        ...
        try:
                ...
                for col, encoder_data in self.encoders.items():
                    if col in df.columns:
                        ...
                        if isinstance(encoder_data, dict) and encoder_data.get('encoding_type') == 'one_hot':
                            ...
                            # Fall back to label encoding for compatibility
                            mapping = encoder_data.get('mappings', encoder_data)
            ...
            try:
                ...
                if scaler.load_scaler('artifacts/scale'):
                    # Apply scaling to the specified columns
                    scaling_columns = scaler.scaling_metadata['columns_to_scale']
                    available_scaling_columns = [col for col in scaling_columns if col in df.columns]
                    
                    for col in available_scaling_columns:
                        if col in scaler.scaling_metadata['scaler_params']:
                            params = scaler.scaling_metadata['scaler_params'][col]
                            original_min = params['min'][0]
                            original_max = params['max'][0]
                            original_value = df[col].iloc[0]
                            
                            # Apply MinMax scaling: (x - min) / (max - min)
                            scaled_value = (original_value - original_min) / (original_max - original_min)
                            df[col] = scaled_value
                            
                            logger.info(f"  ✓ Transformed '{col}': {original_value} → {scaled_value:.4f}")
                else:
                    logger.warning("  ⚠ Could not load scaler artifacts - skipping scaling")
            except Exception as e:
            ...
            # CRITICAL: Reorder columns to match training data for model compatibility
            expected_columns = [

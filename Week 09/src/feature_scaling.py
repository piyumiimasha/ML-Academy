...
class MinMaxScalingStrategy(FeatureScalingStrategy):
    ...
    def save_scaler(self, columns_to_scale: List[str], save_dir: str = 'artifacts/scale') -> bool:
        ...
        try:
            ...
            # Save scaler models (PySpark pipeline models)
            model_path = os.path.join(save_dir, 'spark_scaler_models')
            os.makedirs(model_path, exist_ok=True)
            
            # Save each scaler model separately
            for col, scaler_model in self.scaler_models.items():
                col_model_path = os.path.join(model_path, f"{col}_scaler")
                # Note: Individual MinMaxScaler models don't have save method
                # We'll save the parameters instead
                scaler_params = {
                    'originalMin': scaler_model.originalMin.toArray().tolist() if hasattr(scaler_model.originalMin, 'toArray') else [float(scaler_model.originalMin)],
                    'originalMax': scaler_model.originalMax.toArray().tolist() if hasattr(scaler_model.originalMax, 'toArray') else [float(scaler_model.originalMax)]
                }
                
                with open(f"{col_model_path}_params.json", 'w') as f:
            ...
            metadata = {
                ...
                'framework': 'pyspark',
                'scaler_params': {col: {
                    'min': scaler_model.originalMin.toArray().tolist() if hasattr(scaler_model.originalMin, 'toArray') else [float(scaler_model.originalMin)],
                    'max': scaler_model.originalMax.toArray().tolist() if hasattr(scaler_model.originalMax, 'toArray') else [float(scaler_model.originalMax)]
                } for col, scaler_model in self.scaler_models.items()}
            }
    ...
    def load_scaler(self, save_dir: str = 'artifacts/scale') -> bool:
        """
        Load the fitted scaler metadata for inference.
        ...
        try:
            ...
            with open(metadata_path, 'r') as f:
                self.scaling_metadata = json.load(f)
            
            logger.info(f"✓ Scaler metadata loaded from {save_dir}")
            logger.info(f"  • Scaling columns: {self.scaling_metadata['columns_to_scale']}")
            logger.info(f"  • Framework: {self.scaling_metadata['framework']}")
            return True
            
        except Exception as e:
            logger.error(f"✗ Failed to load scaler metadata: {str(e)}")
            return False
    
    def transform(self, df: DataFrame, columns_to_scale: List[str]) -> DataFrame:
        """
        Apply the loaded scaler parameters to transform data (no fitting).
        ...
        Returns:
            DataFrame with scaled features
        """
        if not hasattr(self, 'scaling_metadata'):
            raise ValueError("Scaler metadata not loaded. Call load_scaler() first.")
        
        logger.info("Applying pre-fitted scaling parameters for inference...")
        
        df_scaled = df
        
        for col in columns_to_scale:
            if col in self.scaling_metadata['scaler_params']:
                params = self.scaling_metadata['scaler_params'][col]
                original_min = params['min'][0]  # Extract scalar value
                original_max = params['max'][0]  # Extract scalar value
                
                # Apply MinMax scaling formula: (x - min) / (max - min)
                df_scaled = df_scaled.withColumn(
                    col,
                    (F.col(col) - original_min) / (original_max - original_min)
                )
                
                logger.info(f"  ✓ Applied scaling to '{col}': min={original_min:.4f}, max={original_max:.4f}")
            else:
